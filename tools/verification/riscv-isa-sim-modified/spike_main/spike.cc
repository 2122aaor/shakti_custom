...
static void help()
{
  ...
  fprintf(stderr, "  -s                    Dumps the register values after every instruction to dump.txt\n"); // nandu
  fprintf(stderr, "  --ms=<value>          Starting address of memory to be dumped\n"); // nandu
  fprintf(stderr, "  --me=<value>          Ending address of memory to be dumped\n"); // nandu
...
int main(int argc, char** argv)
{
  ...
  bool log = false;
  bool risc_dump_flag = false;
  int risc_dump_type= 0;
  int memDumpFlag=0;
  ...
  parser.option('s', 0, 0, [&](const char* s){debug = false; risc_dump_flag=true;}); // nandu
  parser.option(0, "ms", 1, [&](const char* s){mem_start = atoi(s); memDumpFlag++;});
  parser.option(0, "me", 1, [&](const char* s){mem_end = atoi(s); memDumpFlag++;});
  ...
  s.set_risc_dump_flag(risc_dump_flag,risc_dump_type,memDumpFlag,mem_start,mem_end);  
  s.set_debug(debug);
  s.set_log(log);
