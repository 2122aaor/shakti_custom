...
void load_objdump (uint64_t n_bytes, void *memp)
{
    ...
    uint16_t *p2;	    // Instruction Pointer
    uint32_t *p4;           // Word pointer
    uint16_t  instr;	    // RVC - 16-bit instructions only
    uint32_t  data;	    // RVC - 32-bit data
	...
	if (access_width == 2) n = sscanf (line, "%" SCNx64 ":%x", & addr, & instr);
	else if (access_width == 4) n = sscanf (line, "%" SCNx64 ":%x", & addr, & data);
	if (n == 2) {
...
uint32_t  c_read (uint64_t addr, uint64_t n_bytes)
{
    uint8_t  *p1;
    uint16_t *p2;
    uint32_t *p4, result = 0;
    ...
    switch (n_bytes) {
      ...
      //case 8: p8 = U.xp; result = *p8; break;		// #rvc - 64-bit data read not supported
...
uint32_t  c_read_fn (uint64_t addr, uint64_t n_bytes)
{
...
void c_write (uint64_t addr, uint32_t x, uint64_t n_bytes)	
{
    uint8_t  *p1;
    uint16_t *p2;
    uint32_t *p4;
    // uint64_t *p8;		// #rvc - 64-bit data not supported
    ...
    switch (n_bytes) {
      ...
      // case 8: p8 = U.xp; *p8 = x; break;		// #rvc - 64-bit data not supported
      default: fprintf (stderr,
			"ERROR: c_read: n_bytes %0" PRId64 " should be 1, 2, 4 or 8\n",
			n_bytes);
    }
...
uint32_t cmd_bogus      = 0xfFFFfFFFlu;

uint32_t cmd_continue   = 0;
uint32_t cmd_dump       = 1;
uint32_t cmd_quit       = 2;
uint32_t cmd_reset      = 3;
uint32_t cmd_step       = 4;
uint32_t cmd_step_until = 5;
uint32_t cmd_verbosity  = 6;    // arg: verbosity level
uint32_t cmd_dump_mem   = 7;
...
void c_get_console_command (uint32_t *cmd_vec)
{
    char line [256], *p;
	...
	case 'S':
	  ...
	  if (status == 1) {
	      ...
	      cmd_vec [2] = ( (arg1 == 0) ? UINT32_MAX : arg1 );
	  }
	  break;
