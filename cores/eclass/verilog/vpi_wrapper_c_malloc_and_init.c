 ...
 *     #include "vpi_wrapper_c_malloc_and_init.h"
 *     void (*vlog_startup_routines[])() = { c_malloc_and_init_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_malloc_and_init call=c_malloc_and_init_calltf size=64 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_malloc_and_init vpiSysFuncSized 64 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned long long c_malloc_and_init(unsigned long long , unsigned long long );
...
PLI_INT32 c_malloc_and_init_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = c_malloc_and_init(arg_1, arg_2);
...
/* sft: $imported_c_malloc_and_init vpiSysFuncSized 64 unsigned */

/* tab: $imported_c_malloc_and_init call=c_malloc_and_init_calltf size=64 acc=rw:%TASK */

PLI_INT32 c_malloc_and_init_sizetf(PLI_BYTE8 *user_data)
{
  return 64u;
}
...
void c_malloc_and_init_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_malloc_and_init";
  tf_data.calltf = c_malloc_and_init_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_malloc_and_init_sizetf;
  tf_data.user_data = "$imported_c_malloc_and_init";
