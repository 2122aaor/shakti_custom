...
extern uint32_t  c_read (uint64_t addr, uint64_t n_bytes);		

extern uint32_t  c_read_fn (uint64_t addr, uint64_t n_bytes);	

extern void c_write (uint64_t addr, uint32_t x, uint64_t n_bytes);

extern void c_get_console_command (uint32_t *cmd_vec);
