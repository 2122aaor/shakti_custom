...
volatile bool risc_dump_flag= false; // nandu
extern volatile int dumpMode; // nandu
volatile bool ctrlc_pressed = false;
uint64_t mem_start, mem_end; // nandu
int memDumpFlag; // nandu
std::string current_instruction; //nandu
...
void sim_t::main()
{
  ...
  while (!done())
  {
    ...
    if (gdbserver) {
  	...
  	if (memDumpFlag==2){ // nandu
		fprintf(stderr, "\nDumping Memory\n"); // nandu
		debug_mmu->memory_dump(mem_start,mem_end);    // nandu
	}
}
...
void sim_t::set_risc_dump_flag(bool dmp_flg,int typ, int memDmpFlg, int64_t mem_strt, int64_t mem_ed) // nandu
{
  risc_dump_flag = dmp_flg; // nandu
  dumpMode=typ; // nandu
  memDumpFlag=memDmpFlg; // nandu
  mem_start=mem_strt; // nandu
  mem_end=mem_ed; // nandu
} // nandu
