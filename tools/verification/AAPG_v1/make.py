...
def compileASM():
  ...
  subprocess.call('riscv32-unknown-elf-gcc -mcmodel=medany -fomit-frame-pointer -fno-strict-aliasing -fno-builtin -c -o output.o output.s',shell=True)

def linkObject():
  print('linking')
  subprocess.call('riscv32-unknown-elf-ld output.o -T ../link.ld -o output.riscv ',shell=True)

def genObjdump():
    print('objdump')
    subprocess.call('riscv32-unknown-elf-objdump -D output.riscv > disaassembled.txt',shell=True)
    subprocess.call('riscv32-unknown-elf-objdump -s output.riscv > objdump.txt',shell=True)
    subprocess.call('elf2hex 4 65536 output.riscv 2147483648 > code.hex',shell=True);

def runSpike():
    subprocess.call('spike -s --isa=RV{0}IMAFDC output.riscv'.format(bitwidth),shell=True)
    subprocess.call("sed -i '1,66d' dump_raw.txt",shell=True)
    subprocess.call("sed -i '1,68d' dump_raw2.txt",shell=True)
