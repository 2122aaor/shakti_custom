...
def compileASM():
  ...
  subprocess.call('riscv64-unknown-elf-gcc -mcmodel=medany -fomit-frame-pointer -fno-strict-aliasing -fno-builtin -c -o output.o output.s',shell=True)

def linkObject():
  print('linking')
  subprocess.call('riscv64-unknown-elf-ld output.o -T ../link.ld -o output.riscv ',shell=True)
...
def genObjdump():
    ...
    subprocess.call('riscv64-unknown-elf-objcopy -O verilog -j .text -j .tohost -j .data --gap-fill 00000000 --reverse-bytes=4 output.riscv temphex',shell=True);
    subprocess.call("sed -i 's/ //g' temphex",shell=True)
...
try:
  ...
  if(makeType=='all'):
    for seed in seeds:
      ...
      subprocess.call("sed -i '1,68d' spike_fregister_dump.txt",shell=True)
      subprocess.getoutput("diff -iw rtl_register_dump.txt spike_register_dump.txt > 1")
      subprocess.getoutput("sdiff -iW rtl_register_dump.txt spike_register_dump.txt > 2")
      stdoutdata=subprocess.getoutput("head -n -34 1")
      if not stdoutdata:
